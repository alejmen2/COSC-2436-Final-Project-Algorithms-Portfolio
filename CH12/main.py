...
def extract_features(item):
    ...
    """
    # Step 1: read the two numeric fields off the dict.
    size = item["size"]
    redness = item["redness"]
    # Step 2: return them as a plain list -- this list IS the point.
    return [size, redness]
...
def euclidean_distance(a, b):
    ...
    Written out with a loop -- this is just the Pythagorean theorem
    generalized to more than two dimensions.
    """
    total = 0
    # Step 1 + 2: add up the squared difference of each matching pair.
    for i in range(len(a)):
        diff = a[i] - b[i]
        total += diff ** 2
    # Step 3: square root of the sum of squared differences.
...
def k_nearest(training_set, new_item, k):
    ...
    """
    # The key function below computes the Euclidean distance from each
    # training item's features to new_item's features. sorted() then
    # orders the whole training set from nearest to farthest, exactly
    # like the Chapter 2 selection-sort idea but with distance as the
    # sort key instead of a plain number.
    sorted_by_distance = sorted(
        training_set,
        key=lambda item: euclidean_distance(item["features"], new_item["features"]),
    )
    # Only the first k (closest) items matter.
...
def classify(neighbors):
    ...
    """
    # Step 1: tally up how often each label shows up.
    counts = {}
    for neighbor in neighbors:
        label = neighbor["label"]
        counts[label] = counts.get(label, 0) + 1

    # Step 2: find the label with the highest count.
    best_label = None
    best_count = -1
...
# The gram-scale numbers are so much larger than the 1-5 numbers that raw
# Euclidean distance is decided almost entirely by weight.
TRAINING_SET_B_RAW = [
    {"features": [150, 4], "label": "healthy"},
    {"features": [160, 5], "label": "healthy"},
    {"features": [140, 4], "label": "healthy"},
    {"features": [155, 5], "label": "healthy"},
    {"features": [700, 2], "label": "junk"},
    {"features": [750, 1], "label": "junk"},
    {"features": [680, 2], "label": "junk"},
    {"features": [720, 1], "label": "junk"},
]

TEST_POINT_B = {"features": [500, 5]}
...
# A small hardcoded table of users and the movies they've rated (1-5 stars).
USERS = [
    {"name": "Alice", "ratings": {"Inception": 5, "Titanic": 3, "Avatar": 4, "Up": 5}},
    {"name": "Bob", "ratings": {"Inception": 4, "Titanic": 2, "Avatar": 5, "Up": 4}},
    {"name": "Carol", "ratings": {"Inception": 1, "Titanic": 5, "Avatar": 2, "Up": 2}},
    {"name": "Dave", "ratings": {"Inception": 2, "Titanic": 4, "Avatar": 1, "Up": 3}},
    {"name": "Eve", "ratings": {"Inception": 5, "Titanic": 2, "Avatar": 4, "Up": 5}},
]

# Frank hasn't seen "Up" yet -- that's the rating we want to predict.
TARGET_USER = {"name": "Frank", "ratings": {"Inception": 5, "Titanic": 3, "Avatar": 4}}
...
def recommend(user, users, k):
    ...
    """
    # Find every movie that shows up in the users table.
    all_movies = set()
    for other in users:
        all_movies.update(other["ratings"].keys())

    # The movie(s) the target user hasn't rated yet -- we predict the first one.
    known_movies = list(user["ratings"].keys())
    unrated_movies = [m for m in all_movies if m not in user["ratings"]]
    if not unrated_movies:
        raise ValueError("Target user has already rated every movie.")
    movie_to_predict = unrated_movies[0]

    # Build a feature vector for the target user from the movies they HAVE rated.
    shared_movies = known_movies
    user_features = [user["ratings"][m] for m in shared_movies]

    # Build the same kind of feature vector for every other user who has
    # rated the movie we're trying to predict.
    candidates = []
    for other in users:
        if movie_to_predict not in other["ratings"]:
            continue  # can't use this person as a neighbor if they never rated the movie
        other_features = [other["ratings"].get(m, 0) for m in shared_movies]
        candidates.append({
            "features": other_features,
            "value": other["ratings"][movie_to_predict],
            "name": other["name"],
        })

    # Reuse the exact same k_nearest() used for classification, then feed
    # its output straight into predict_rating() -- the regression ending.
    neighbors = k_nearest(candidates, {"features": user_features}, k)
    predicted_rating = predict_rating(neighbors)
    return movie_to_predict, predicted_rating
...
def main():
    ...
    neighbors_b_raw = k_nearest(TRAINING_SET_B_RAW, TEST_POINT_B, 3)
    ...
    combined = TRAINING_SET_B_RAW + [{"features": TEST_POINT_B["features"]}]
    ...
    movie, predicted_rating = recommend(TARGET_USER, USERS, 3)
    print(f"Predicted rating for {TARGET_USER['name']} on '{movie}': {predicted_rating:.2f}")
