...
def dijkstra(graph, start, finish):
    ...
    while node is not None:
        ...
        # `processed` is required because Dijkstra's algorithm is greedy:
        # once a node is processed we are declaring "this is the final,
        # cheapest way to reach it." Without this list, the same
        # already-finalized node could be picked again forever (its cost
        # never grows on its own), so the loop would never terminate and
        # we'd redo the same relaxations infinitely instead of making
        # progress toward unprocessed nodes.
...
def bfs_shortest_path(graph, start, finish):
    ...
    FEWEST EDGES (hops) on an unweighted graph (dict-of-lists).
    """
    queue = deque([start])
    visited = set([start])
    parents = {start: None}
